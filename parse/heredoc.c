...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/08/19 20:18:39 by udemirci          #+#    #+#             */
/*   Updated: 2025/08/20 01:10:04 by udemirci         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	handle_heredoc_input(int pipe_fd, char *limiter, int expand_flag,
		t_cmds *cmd)
{
	char	*line;
	char	*expanded;
...
static void	run_child_process(int pipe_fd[2], char *limiter, int expand_flag,
		t_cmds *cmd)
{
...
static int	open_heredoc_pipe(char *limiter, int expand_flag, t_cmds *cmd)
{
	int		pipe_fd[2];
	pid_t	pid;
...
int	setup_heredocs(t_cmds *cmds, t_env *env)
{
	...
	while (current)
	{
		...
		while (redir)
		{
			if (redir->type == 4)
			{
				redir->heredoc_fd = open_heredoc_pipe(redir->file,
						redir->heredoc_expand_flag, current);
